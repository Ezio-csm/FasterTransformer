...
def test_gemmAllreduce(bs_seq_len, hidden_size, intermediate_size, dtype, my_handle, warp_up=5, iter_cnt=200, nStream=2):
    ...
    single_bs_seq_len = bs_seq_len // world_size
    single_hidden_size = hidden_size // world_size

    data = torch.randn(bs_seq_len, intermediate_size, dtype=dtype, device=f'cuda:{rank}')
    weight = torch.randn(intermediate_size, single_hidden_size, dtype=dtype, device=f'cuda:{rank}')
    torch.distributed.broadcast(data, src=0, group=g_process_group)
    torch.cuda.synchronize()
    sigle_data = data[rank*single_bs_seq_len:(rank+1)*single_bs_seq_len]
    ...
    for _ in range(warp_up):
        data_list = [torch.zeros_like(sigle_data) for _ in range(world_size)]
        torch.distributed.all_gather(data_list, sigle_data, group=g_process_group)
        data_list = torch.cat(data_list, dim=0)
        out = torch.matmul(data_list, weight)
    torch.cuda.synchronize()
    ...
    start_time = time.time()
    for _ in range(iter_cnt):
        my_handle.GemmAllgather(data, weight, out_2, bs_seq_len, single_hidden_size, intermediate_size)
    torch.cuda.synchronize()
    end_time = time.time()
    time2 = (end_time - start_time) / iter_cnt
    if rank == 0:
        print(f'Myop cost time: {(end_time - start_time)/iter_cnt}')
...
if __name__ == "__main__":
    nChunk = 4
    my_handle = torch.classes.FasterTransformer.GemmAllgatherOp(nChunk)
    # time1, time2 = test_gemmAllreduce(bs_seq_len=4096, hidden_size=2048, intermediate_size=2048, dtype=torch.bfloat16, my_handle=my_handle, nStream=2)
    # if rank==0:
    #     print(time1, time2)

    if rank==0:
        df = pd.DataFrame(columns=['bs_seq_len', 'hidden_size', 'intermediate_size', 'dtype', 'time_reference', 'time_myfunc', 'time_pysplit', 'speedup_myfunc', 'speedup_pysplit'])
            ...
            for hidden_size in hidden_size_list:
                for intermediate_size in intermediate_size_lsit:
                    ...
                    if dtype == torch.float32 and hidden_size == 12288 and intermediate_size == 32768:
                        continue
                    time1, time2 = test_gemmAllreduce(bs_seq_len, hidden_size, intermediate_size, dtype, my_handle, nStream=2)
                    if rank==0:
                        log_info = f'bs_seq_len: {bs_seq_len}, hidden_size: {hidden_size}, intermediate_size: {intermediate_size}, dtype: {dtype}, time_reference: {time1}, time_myfunc: {time2}, speedup_myfunc: {time1/time2}'
                        print(log_info)
                        new_row = pd.DataFrame([[bs_seq_len, hidden_size, intermediate_size, dtype, time1, time2, time1/time2]], columns=['bs_seq_len', 'hidden_size', 'intermediate_size', 'dtype', 'time_reference', 'time_myfunc', 'speedup_myfunc'])
                        df = pd.concat([df, new_row], ignore_index=True)
    if rank==0:
        df.to_csv(f'test_{world_size}.csv', index=False)
