...
def test_gemmAllreduce(bs_seq_len, hidden_size, intermediate_size, dtype, my_handle, warp_up=5, iter_cnt=200, nStream=2):
    ...
    single_seq = bs_seq_len // world_size
    ...
    for _ in range(warp_up):
        ...
        if world_size > 1:
            for r in range(world_size):
                torch.distributed.reduce(out[r*single_seq:(r+1)*single_seq],
                                         dst=r, group=g_process_group)
    ...
    start_time = time.time()
    for _ in range(iter_cnt):
        my_handle.GemmReduceScatter(data, weight, out_2, bs_seq_len, hidden_size, single_intermediate_size)
    torch.cuda.synchronize()
    end_time = time.time()
    ...
    out = out[rank*single_seq:(rank+1)*single_seq]
    out_2 = out_2[rank*single_seq:(rank+1)*single_seq]
    # if rank == 0:
    #     print(f'out:{out}, \nsum:{out.sum()}')
    #     print(f'out2: {out_2}, \nsum:{out_2.sum()}')
...
if __name__ == "__main__":
    nChunk = 4
    my_handle = torch.classes.FasterTransformer.GemmReduceScatterOp(nChunk)
    ...
    if rank==0:
        df.to_csv(f'test_{world_size}_reducescatter.csv', index=False)
